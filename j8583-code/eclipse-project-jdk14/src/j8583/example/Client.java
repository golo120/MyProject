...
public class Client implements Runnable {
	...
	public void run() {
		...
		try {
				...
				if (sock.getInputStream().read(buf) == size) {
					try {
						...
						if (f48 != null && f48.getValue() instanceof ProductData) {
							...
							log.debug("Field 48 encoded: '" + f48 + "' pid:" + v.getProductId() + " cat:"
								+ v.getCategoryId());
						}
			...
			if (done) {
				log.info("Socket closed because we're done (" + pending.size() + " pending)");
			} else {
				log.error("Reading responses, " + pending.size() + " pending", ex);
				try {
					sock.close();
...
//		mfact.setTraceNumberGenerator(new SimpleTraceGenerator((int)(System.currentTimeMillis() % 10000)));
		...
		for (int i = 0; i < 10; i++) {
			...
			for(int j = 1; j < 64; j++){
				IsoValue mIsoValue = req.getField(j);
				if(mIsoValue == null){
					continue;
				}else{
					log.debug("-------golo,field:" + j + " , value=" + mIsoValue.toString());
				}
			}
